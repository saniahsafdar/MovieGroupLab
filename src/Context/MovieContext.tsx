...
export function MovieContextProvider({ children }: { children: ReactNode }) {
  ...
  function addMovie(movie: Movie): void {
    if (isOnList(movie)) {
      console.log(`${movie.title} is already on your list ${movie.id} `);
    } else {
  ...
  function isOnList(movie: Movie) {
    return favoriteMovies.some((existingMovie) => existingMovie.id === movie.id);
  }

  return (
    <MovieContext.Provider
      value={{ favoriteMovies, addMovie, removeMovie, isOnList }}
    >
      {children}
    </MovieContext.Provider>
  );
